...
function main(){
  ...
  sheet.clear()
  var page = 1; // for every other page, increment this variable.
  var testIt = ImportJSON("call Import.io connector API here","/results/image,/results/cost,/results/author,/results/link,/results/rating", "noInherit,noTruncate,noHeaders");	// making a request to import.io connector api
  
    var i = testIt.length - 1;
    while (i > -1) { // fill the spreadsheet with the scraped data.
      var item = testIt[i--];
      sheet.insertRowBefore(2);
      sheet.getRange('A1').setValue('Image');
      sheet.getRange('B1').setValue('Cost');
      sheet.getRange('C1').setValue('Author');
      sheet.getRange('D1').setValue('Title');
      sheet.getRange('E1').setValue('Link');
      sheet.getRange('F1').setValue('Rating');
      sheet.getRange('A2').setValues([[item[0]]]);
      sheet.getRange('B2').setValues([[item[1]]]);
      sheet.getRange('C2').setValues([[item[2]]]);
      sheet.getRange('D2').setValues([[item[3]]]);
      sheet.getRange('E2').setValues([[item[4]]]);
      if (item[5]==""){
        sheet.getRange('F2').setValue("Not rated.");
      }else{
        sheet.getRange('F2').setValues([[item[5]]]);
      }  
    }
